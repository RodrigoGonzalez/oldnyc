...
def parse_month(mon):
  """Takes "Jan" -> 1"""
  return int(time.strptime(mon[:3], "%b")[1])
...
class Record:
  ...
  def FromString(str):
    ...
    r.tabular = {}
    if m := re.search(
        r'<!-- BEGIN INNER BIB TABLE -->(.*?)<!-- END INNER BIB TABLE -->',
        str,
        re.DOTALL,
    ):
      ...
      for field_html in fields_html[1:]:
        ...
        if tag_name := ExtractRegex(r'class="bibInfoLabel">([^<]+)', field_html):
  ...
  def _single_table(self, tag):
    ...
    return strip_tags(vals[0]) if len(vals) == 1 else ''
  ...
  def ExtractDateRange(raw_txt):
    ...
    if year := ExtractRegex(r'^(\d{4})$', txt):
      start = date(int(year), 1, 1)
      end = date(int(year), 12, 31)
      return [start, end]

    if ca_year := ExtractRegex(r'^ca? ?(\d{4})$', txt):
      y = int(ca_year)
      start = date(y - 1, 1, 1)
      end = date(y + 1, 12, 31)
      return [start, end]

    if ca_year := ExtractRegex(r'^(\d{4})\?$', txt):
      y = int(ca_year)
      start = date(y - 1, 1, 1)
      end = date(y + 1, 12, 31)
      return [start, end]
    ...
    if m := re.match(r'^(\d{4}) ([A-Z][a-z]{2,3}) ?(\d{1,2})$', txt):
    ...
    if m := re.match(r'^(\d{4}) ([A-Z][a-z]{2,3}) ?(\d{1,2})-(\d{1,2})$', txt):
    ...
    if m := re.match(r'^(\d{4}) ([A-Z][a-z]{2,3})$', txt):
      year, mon = m.groups()
    ...
    if m := re.match(r'^([A-Z][a-z]{2,3}) (\d{4})$', txt):
      mon, year = m.groups()
      year, mon = int(year), parse_month(mon)
      start = date(year, mon, 1)
      # This monstrosity determines the last day of the month
    ...
    if dec := ExtractRegex(r'^([12]\d\d)-$', txt):
      year = int(f'{dec}0')
      start = date(year, 1, 1)
    ...
    if yr := re.search(r'^(\d{4}) *- *(\d{2,4})$', txt):
    ...
    if yp := re.search(r'^(\d{4}) or (\d{4})$', txt):
      start = int(yp.group(1))
      end = int(yp.group(2))
      return [date(start, 1, 1), date(end, 12, 31)]

    if bt := re.search(r'^between (.*) and (.*)$', txt, re.IGNORECASE):
      left = Record.ExtractDateRange(bt.group(1))
      right = Record.ExtractDateRange(bt.group(2))
      if left and right:
        return [left[0], right[1]]

    if cen := ExtractRegex(r'^([12]\d)--$', txt):
      year = int(f'{cen}00')
      start = date(year, 1, 1)
  ...
  def date_range(self):
    ...
    if self._date_range[0] is None:
      self._date_range[0] = date(1850, 1, 1)
    if self._date_range[1] is None:
  ...
  def locatable_str(self):
    ...
    return addr if (addr := ExtractAddress(self.title())) else None
...
def CleanFolder(folder):
  ...
  if folder[-1] == '.' and folder[-3] != '.':  # e.g. 'Y.M.C.A'
    folder = folder[:-1]
...
def ExtractAddress(str):
  ...
  for addr_match in re.finditer(pat, str):
    ...
    if (1850 < num < 2000 and re.search(
        r'(?i)(?:\bst.?\b)|(?:\bave\.?\b)|street|avenue|Broadway|Laidley|Pine|Geary|Sloat|Bush|Mission|Market|Broderick|Washington|Van Ness',
        addr,
    ) or not 1850 < num < 2000):
      return addr
  ...
  pat = f'''((?: [A-Z](?:[a-z.]|'[A-Za-z])+)+) (?:and|&|at)((?: [A-Z](?:[a-z.]|'[A-Za-z])+)+)( {street_types})'''
  for addr_match in re.finditer(pat, str):
